...
public class CloudStructure extends Structure {
    public static final Codec<CloudStructure> CODEC = RecordCodecBuilder.create((p_229075_) -> p_229075_.group(settingsCodec(p_229075_),
    ...
    ).apply(p_229075_, CloudStructure::new));

    private final BlockStateProvider blocks;
    private final int size;
    private final int rangeY;

    public CloudStructure(StructureSettings settings, BlockStateProvider blocks, int size, int rangeY) {
        super(settings);
        this.blocks = blocks;
        this.size = size;
        this.rangeY = rangeY;
    ...
    public Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        return Structure.onTopOfChunkCenter(context, Heightmap.Types.OCEAN_FLOOR_WG, (builder) -> generatePieces(builder, context, this.blocks, this.size, this.rangeY));
    }
    ...
    private static void generatePieces(StructurePiecesBuilder builder, GenerationContext context, BlockStateProvider blocks, int size, int rangeY) {
        ...
        chunks.forEach((chunkPos, blockPosSet) -> {
            BoundingBox boundingBox = new BoundingBox(
                    ...
                    Math.max(initialY - 16, 0),
                    chunkPos.getMinBlockZ(),
                    chunkPos.getMaxBlockX(),
                    finalY + 255,
                    chunkPos.getMaxBlockZ());
            builder.addPiece(new CloudChunk(blockPosSet, blocks, boundingBox, orientation));
        });
    }

    @Override
    public StructureType<?> type() {
        return ReachStructureTypes.LARGE_CLOUD.get();
    }
}
