...
import net.nullean.reach.world.feature.configurations.ShardConfiguration;

import javax.annotation.Nullable;
import java.util.Optional;
import java.util.Random;

public class ShardLargeFeature extends Feature<ShardConfiguration>
{
    public static Block block1;

    public ShardLargeFeature(Codec<ShardConfiguration> p_159960_, Block block)
    {
        super(p_159960_);
        block1 = block;
    }

    public boolean place(FeaturePlaceContext<ShardConfiguration> p_159967_)
    {
        WorldGenLevel worldgenlevel = p_159967_.level();
        BlockPos blockpos = p_159967_.origin();
        ShardConfiguration Shardconfiguration = p_159967_.config();
        ...
        {
            Optional<Column> optional = Column.scan(worldgenlevel, blockpos, Shardconfiguration.floorToCeilingSearchRange, ShardUtils::isEmptyOrWater, ShardUtils::isShardBaseOrLava);
            if (optional.isPresent() && optional.get() instanceof Column.Range)
            {
                ...
                {
                    int i = (int)((float)column$range.height() * Shardconfiguration.maxColumnRadiusToCaveHeightRatio);
                    int j = Mth.clamp(i, Shardconfiguration.columnRadius.getMinValue(), Shardconfiguration.columnRadius.getMaxValue());
                    int k = Mth.randomBetweenInclusive(random, Shardconfiguration.columnRadius.getMinValue(), j);
                    ShardLargeFeature.LargeShard Shardfeature$Shard = makeShard(blockpos.atY(column$range.ceiling() - 1), false, random, k, Shardconfiguration.stalactiteBluntness, Shardconfiguration.heightScale);
                    ShardLargeFeature.LargeShard Shardfeature$Shard1 = makeShard(blockpos.atY(column$range.floor() + 1), true, random, k, Shardconfiguration.stalagmiteBluntness, Shardconfiguration.heightScale);
                    ShardLargeFeature.WindOffsetter Shardfeature$windoffsetter;
                    if (Shardfeature$Shard.isSuitableForWind(Shardconfiguration) && Shardfeature$Shard1.isSuitableForWind(Shardconfiguration)) {
                        Shardfeature$windoffsetter = new ShardLargeFeature.WindOffsetter(blockpos.getY(), random, Shardconfiguration.windSpeed);
                    }
                    else
                    {
                        Shardfeature$windoffsetter = ShardLargeFeature.WindOffsetter.noWind();
                    }
    ...
    private static ShardLargeFeature.LargeShard makeShard(BlockPos p_159969_, boolean p_159970_, RandomSource p_159971_, int p_159972_, FloatProvider p_159973_, FloatProvider p_159974_)
    {
        return new ShardLargeFeature.LargeShard(p_159969_, p_159970_, p_159972_, (double)p_159973_.sample(p_159971_), (double)p_159974_.sample(p_159971_));
    }
    ...
    static final class LargeShard
    {
        ...
        boolean moveBackUntilBaseIsInsideStoneAndShrinkRadiusIfNecessary(WorldGenLevel p_159990_, ShardLargeFeature.WindOffsetter p_159991_)
        {
            while(this.radius > 1)
            {
        ...
        void placeBlocks(WorldGenLevel p_159993_, RandomSource p_159994_, ShardLargeFeature.WindOffsetter p_159995_)
        {
                    ...
                    if (!(f > (float)this.radius))
                    {
                        ...
                        if (k > 0)
                        {
                            ...
                            for(int l = 0; l < k; ++l)
                            {
                                ...
                                {
                                    flag = true;
                                    Block block = ShardLargeFeature.block1;
        ...
        boolean isSuitableForWind(ShardConfiguration p_159997_) {
    ...
    static final class WindOffsetter {
        ...
        static ShardLargeFeature.WindOffsetter noWind() {
            return new ShardLargeFeature.WindOffsetter();
        }
