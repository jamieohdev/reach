...
import net.nullean.reach.world.feature.configurations.ShardConfiguration1;

import javax.annotation.Nullable;
import java.util.Optional;
import java.util.Random;

public class ShardLargeFeature1 extends Feature<ShardConfiguration1>
{
    public static Block block1;

    public ShardLargeFeature1(Codec<ShardConfiguration1> p_159960_, Block block)
    {
        super(p_159960_);
        block1 = block;
    }

    public boolean place(FeaturePlaceContext<ShardConfiguration1> p_159967_)
    {
        WorldGenLevel worldgenlevel = p_159967_.level();
        BlockPos blockpos = p_159967_.origin();
        ShardConfiguration1 ShardConfiguration1 = p_159967_.config();
        ...
        {
            Optional<Column> optional = Column.scan(worldgenlevel, blockpos, ShardConfiguration1.floorToCeilingSearchRange, ShardUtils::isEmptyOrWater, ShardUtils::isShardBaseOrLava);
            if (optional.isPresent() && optional.get() instanceof Column.Range)
            {
                ...
                {
                    int i = (int)((float)column$range.height() * ShardConfiguration1.maxColumnRadiusToCaveHeightRatio);
                    int j = Mth.clamp(i, ShardConfiguration1.columnRadius.getMinValue(), ShardConfiguration1.columnRadius.getMaxValue());
                    int k = Mth.randomBetweenInclusive(random, ShardConfiguration1.columnRadius.getMinValue(), j);
                    ShardLargeFeature1.LargeShard Shardfeature$Shard = makeShard(blockpos.atY(column$range.ceiling() - 1), false, random, k, ShardConfiguration1.stalactiteBluntness, ShardConfiguration1.heightScale);
                    ShardLargeFeature1.LargeShard Shardfeature$Shard1 = makeShard(blockpos.atY(column$range.floor() + 1), true, random, k, ShardConfiguration1.stalagmiteBluntness, ShardConfiguration1.heightScale);
                    ShardLargeFeature1.WindOffsetter Shardfeature$windoffsetter;
                    if (Shardfeature$Shard.isSuitableForWind(ShardConfiguration1) && Shardfeature$Shard1.isSuitableForWind(ShardConfiguration1)) {
                        Shardfeature$windoffsetter = new ShardLargeFeature1.WindOffsetter(blockpos.getY(), random, ShardConfiguration1.windSpeed);
                    }
                    else
                    {
                        Shardfeature$windoffsetter = ShardLargeFeature1.WindOffsetter.noWind();
                    }
    ...
    private static ShardLargeFeature1.LargeShard makeShard(BlockPos p_159969_, boolean p_159970_, RandomSource p_159971_, int p_159972_, FloatProvider p_159973_, FloatProvider p_159974_)
    {
        return new ShardLargeFeature1.LargeShard(p_159969_, p_159970_, p_159972_, (double)p_159973_.sample(p_159971_), (double)p_159974_.sample(p_159971_));
    }
    ...
    static final class LargeShard
    {
        ...
        boolean moveBackUntilBaseIsInsideStoneAndShrinkRadiusIfNecessary(WorldGenLevel p_159990_, ShardLargeFeature1.WindOffsetter p_159991_)
        {
            while(this.radius > 1)
            {
        ...
        void placeBlocks(WorldGenLevel p_159993_, RandomSource p_159994_, ShardLargeFeature1.WindOffsetter p_159995_)
        {
                    ...
                    if (!(f > (float)this.radius))
                    {
                        ...
                        if (k > 0)
                        {
                            ...
                            for(int l = 0; l < k; ++l)
                            {
                                ...
                                {
                                    flag = true;
                                    Block block = ShardLargeFeature1.block1;
        ...
        boolean isSuitableForWind(ShardConfiguration1 p_159997_) {
    ...
    static final class WindOffsetter {
        ...
        static ShardLargeFeature1.WindOffsetter noWind() {
            return new ShardLargeFeature1.WindOffsetter();
        }
