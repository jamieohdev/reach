...
public class BlemishstoneClusterFeature extends Feature<BlemishstoneClusterConfiguration> {
   public BlemishstoneClusterFeature(Codec<BlemishstoneClusterConfiguration> p_159575_) {
      super(p_159575_);
   }

   public boolean place(FeaturePlaceContext<BlemishstoneClusterConfiguration> p_159605_) {
      WorldGenLevel worldgenlevel = p_159605_.level();
      BlockPos blockpos = p_159605_.origin();
      BlemishstoneClusterConfiguration Blemishstoneclusterconfiguration = p_159605_.config();
      RandomSource randomsource = p_159605_.random();
      if (!BlemishstoneUtils.isEmptyOrWater(worldgenlevel, blockpos)) {
         return false;
      } else {
         int i = Blemishstoneclusterconfiguration.height.sample(randomsource);
         float f = Blemishstoneclusterconfiguration.wetness.sample(randomsource);
         float f1 = Blemishstoneclusterconfiguration.density.sample(randomsource);
         int j = Blemishstoneclusterconfiguration.radius.sample(randomsource);
         int k = Blemishstoneclusterconfiguration.radius.sample(randomsource);

         for(int l = -j; l <= j; ++l) {
            for(int i1 = -k; i1 <= k; ++i1) {
               double d0 = this.getChanceOfStalagmiteOrStalactite(j, k, l, i1, Blemishstoneclusterconfiguration);
               BlockPos blockpos1 = blockpos.offset(l, 0, i1);
               this.placeColumn(worldgenlevel, randomsource, blockpos1, l, i1, f, d0, i, f1, Blemishstoneclusterconfiguration);
            }
         }
   ...
   private void placeColumn(WorldGenLevel p_225016_, RandomSource p_225017_, BlockPos p_225018_, int p_225019_, int p_225020_, float p_225021_, double p_225022_, int p_225023_, float p_225024_, BlemishstoneClusterConfiguration p_225025_) {
      Optional<Column> optional = Column.scan(p_225016_, p_225018_, p_225025_.floorToCeilingSearchRange, BlemishstoneUtils::isEmptyOrWater, BlemishstoneUtils::isNeitherEmptyNorWater);
      if (optional.isPresent()) {
         ...
         if (optionalint.isPresent() || optionalint1.isPresent()) {
            ...
            if (optionalint.isPresent() && flag1 && !this.isLava(p_225016_, p_225018_.atY(optionalint.getAsInt()))) {
               int k = p_225025_.BlemishstoneBlockLayerThickness.sample(p_225017_);
               this.replaceBlocksWithBlemishstoneBlocks(p_225016_, p_225018_.atY(optionalint.getAsInt()), k, Direction.UP);
               int l;
               ...
               j = this.getBlemishstoneHeight(p_225017_, p_225019_, p_225020_, p_225024_, l, p_225025_);
            } else {
               j = 0;
            }
            ...
            if (optionalint2.isPresent() && flag2 && !this.isLava(p_225016_, p_225018_.atY(optionalint2.getAsInt()))) {
               int i1 = p_225025_.BlemishstoneBlockLayerThickness.sample(p_225017_);
               this.replaceBlocksWithBlemishstoneBlocks(p_225016_, p_225018_.atY(optionalint2.getAsInt()), i1, Direction.DOWN);
               if (optionalint.isPresent()) {
                  ...
                  i3 = this.getBlemishstoneHeight(p_225017_, p_225019_, p_225020_, p_225024_, p_225023_, p_225025_);
               }
            } else {
               i3 = 0;
            }
            ...
            if (optionalint.isPresent()) {
               BlemishstoneUtils.growPointedBlemishstone(p_225016_, p_225018_.atY(optionalint.getAsInt() - 1), Direction.DOWN, j3, flag3);
            }

            if (optionalint2.isPresent()) {
               BlemishstoneUtils.growPointedBlemishstone(p_225016_, p_225018_.atY(optionalint2.getAsInt() + 1), Direction.UP, j1, flag3);
            }
   ...
   private int getBlemishstoneHeight(RandomSource p_225009_, int p_225010_, int p_225011_, float p_225012_, int p_225013_, BlemishstoneClusterConfiguration p_225014_) {
   ...
   private boolean canPlacePool(WorldGenLevel p_159620_, BlockPos p_159621_) {
      ...
      if (!blockstate.is(Blocks.WATER) && !blockstate.is(ReachBlocks.BLEMISH_STONE.get()) && !blockstate.is(ReachBlocks.BLEMISH_TEST.get())) {
   ...
   private void replaceBlocksWithBlemishstoneBlocks(WorldGenLevel p_159589_, BlockPos p_159590_, int p_159591_, Direction p_159592_) {
      BlockPos.MutableBlockPos blockpos$mutableblockpos = p_159590_.mutable();

      for(int i = 0; i < p_159591_; ++i) {
         if (!BlemishstoneUtils.placeBlemishstoneBlockIfPossible(p_159589_, blockpos$mutableblockpos)) {
            return;
         }
   ...
   private double getChanceOfStalagmiteOrStalactite(int p_159577_, int p_159578_, int p_159579_, int p_159580_, BlemishstoneClusterConfiguration p_159581_) {
      int i = p_159577_ - Math.abs(p_159579_);
      int j = p_159578_ - Math.abs(p_159580_);
      int k = Math.min(i, j);
      return (double)Mth.clampedMap((float)k, 0.0F, (float)p_159581_.maxDistanceFromEdgeAffectingChanceOfBlemishstoneColumn, p_159581_.chanceOfBlemishstoneColumnAtMaxDistanceFromCenter, 1.0F);
   }
